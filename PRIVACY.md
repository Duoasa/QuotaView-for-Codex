...
The sanitized `usage.json` contains only the plan type, primary rate-window
percentage/duration/reset time, normal Credits balance flags, limit-reached
state, lifetime tokens, the newest daily token bucket, timestamps, protocol
versions, source label, and plugin installation identifier.
